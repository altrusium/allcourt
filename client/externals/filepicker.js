...
filepicker.extend("comm", function () {
    ...
    var d = function (b) {
        if (b.origin != a.urls.BASE && b.origin != a.urls.DIALOG_BASE) return;
...
filepicker.extend("dragdrop", function () {
    ...
    var c = function (c, d) {
        ...
        var o = function () {
            return c && (c.getAttribute("disabled") || "enabled") != "disabled";
        };
        ...
        c.addEventListener("drop", function (b) {
            ...
            if (b.dataTransfer.items) {
                e = b.dataTransfer.items;
                for (d = 0; d < e.length; d++) {
                    f = e[d] && e[d].webkitGetAsEntry ? e[d].webkitGetAsEntry() : undefined;
                    if (f && !! f.isDirectory) {
            ...
            var g = b.dataTransfer.files;
            var i = g.length;
            if (v(g)) {
                h(g);
                c.setAttribute("disabled", "disabled");
                for (d = 0; d < g.length; d++) a.store(g[d], n, r(d, i), s, t(d, i));
            }
            return false;
        });
        var p = {};
            ...
            return function (e) {
                if (!d.multiple) i([e]);
                    ...
                    if (q.length == b) {
                        ...
                        q = [];
                        p = {};
                        c.setAttribute("disabled", "enabled");
                    } else {
                        p[a] = 100;
                        u(b);
                    }
...
filepicker.extend("files", function () {
    ...
    var i = function (b, c, d, e, f) {
        ...
        a.util.setDefault(c, 'location', 'S3');
...
filepicker.extend(function () {
    ...
    var f = function (b, c, e, f) {
        ...
        c = c || function () {};
        e = e || a.errors.handleError;
        a.picker.createPicker(b, c, e, true, false, f);
    };
    var g = function (b, c, e, f, g) {
        d();
        ...
        var h = !! b.multiple;
        var i = !! b ? a.util.clone(b) : {};
        i.storeLocation = c.location || 'S3';
        i.storePath = c.path;
        i.storeContainer = c.container;
        i.storeAccess = c.access || 'private';
        if (h && i.storePath)
            if (i.storePath.charAt(i.storePath.length - 1) != "/") throw new a.FilepickerException("pickAndStore with multiple files requires a path that ends in '/'");
        var j = e;
        if (!h) j = function (a) {
            e([a]);
        };
        a.picker.createPicker(i, j, f, h, false, g);
    };
    ...
    var r = function (b, c, e, f, g, h) {
        ...
        if (e.container) options.storeContainer = e.container;
...
filepicker.extend("picker", function () {
    ...
    var c = function (b, c) {
        var d = function (d) {
            ...
            if (d.error) {
                ...
                if (d.payload.data.container) e.container = d.payload.data.container;
                if (d.payload.data.path) e.path = d.payload.data.path;
                e.isWriteable = true;
                b(e);
            }
            a.modal.close();
    ...
    var f = function (a) {
        ...
        if (c.url) c = c.url;
        b.url = c;
        var d = a.url.data || a.data;
        b.filename = d.filename;
        b.mimetype = d.type;
        b.size = d.size;
        if (d.key) b.key = d.key;
        if (d.container) b.container = d.container;
        if (d.path) b.path = d.path;
        b.isWriteable = true;
        return b;
    };
    var g = function (b, c, d) {
        var e = function (e) {
            if (e.type == "filepickerProgress") {
                if (d) {
                    fpfile = f(e.payload);
                    d(fpfile);
                }
                return;
            } else if (e.type !== "filepickerUrl") return;
            a.uploading = false;
            if (e.error) {
                a.util.console.error(e.error);
                c(a.errors.FPError(102));
            } else {
                var g = [];
                if (!a.util.isArray(e.payload)) e.payload = [e.payload];
                for (var h = 0; h < e.payload.length; h++) {
                    fpfile = f(e.payload[h]);
                    g.push(fpfile);
                }
                b(g);
            }
    ...
    var h = function (f, i, j, k, l, m) {
        ...
        var t;
        if (k) {
            s = a.urls.constructPickUrl(f, n, true);
            t = g(p, q, m);
        } else if (l) {
            s = a.urls.constructPickFolderUrl(f, n);
            t = d(p, q);
        } else {
            s = a.urls.constructPickUrl(f, n, false);
            t = c(p, q);
        }
        a.window.open(f.container, s, r);
        a.handlers.attach(n, t);
        var u = n + "-upload";
        a.handlers.attach(u, e(function () {
            a.handlers.detach(u);
        }));
    };
    return {
...
filepicker.extend("urls", function () {
    ...
    var c = b.replace("www", "dialog");
    var d = c + "/dialog/open/";
    var e = c + "/dialog/save/";
    var f = c + "/dialog/folder/";
    var g = b + "/api/store/";
    var h = function (b, c, e) {
        return d + "?key=" + a.apikey + "&id=" + c + "&referrer=" + window.location.hostname + "&iframe=" + (b.container != 'window') + "&version=" + a.API_VERSION + (b.services ? "&s=" + b.services.join(",") : "") + (e ? "&multi=" + !! e : "") + (b.mimetypes !== undefined ? "&m=" + b.mimetypes.join(",") : "") + (b.extensions !== undefined ? "&ext=" + b.extensions.join(",") : "") + (b.openTo !== undefined ? "&loc=" + b.openTo : "") + (b.maxSize ? "&maxSize=" + b.maxSize : "") + (b.maxFiles ? "&maxFiles=" + b.maxFiles : "") + (b.signature ? "&signature=" + b.signature : "") + (b.policy ? "&policy=" + b.policy : "") + (b.mobile !== undefined ? "&mobile=" + b.mobile : "") + (b.folders !== undefined ? "&folders=" + b.folders : "") + (b.storeLocation ? "&storeLocation=" + b.storeLocation : "") + (b.storePath ? "&storePath=" + b.storePath : "") + (b.storeContainer ? "&storeContainer=" + b.storeContainer : "") + (b.storeAccess ? "&storeAccess=" + b.storeAccess : "");
    };
    var i = function (b, c) {
        return f + "?key=" + a.apikey + "&id=" + c + "&referrer=" + window.location.hostname + "&iframe=" + (b.container != 'window') + "&version=" + a.API_VERSION + (b.services ? "&s=" + b.services.join(",") : "") + (b.openTo !== undefined ? "&loc=" + b.openTo : "") + (b.signature ? "&signature=" + b.signature : "") + (b.policy ? "&policy=" + b.policy : "") + (b.mobile !== undefined ? "&mobile=" + b.mobile : "");
    };
    var j = function (b, c, d) {
        if (b.indexOf("&") >= 0 || b.indexOf("?") >= 0) b = encodeURIComponent(b);
        return e + "?url=" + b + "&key=" + a.apikey + "&id=" + d + "&referrer=" + window.location.hostname + "&iframe=" + (c.container != 'window') + "&version=" + a.API_VERSION + (c.services ? "&s=" + c.services.join(",") : "") + (c.openTo !== undefined ? "&loc=" + c.openTo : "") + (c.mimetype !== undefined ? "&m=" + c.mimetype : "") + (c.extension !== undefined ? "&ext=" + c.extension : "") + (c.mobile !== undefined ? "&mobile=" + c.mobile : "") + (c.suggestedFilename !== undefined ? "&defaultSaveasName=" + c.suggestedFilename : "") + (c.signature ? "&signature=" + c.signature : "") + (c.policy ? "&policy=" + c.policy : "");
    };
    var k = function (b) {
        return g + b.location + "?key=" + a.apikey + (b.base64decode ? "&base64decode=true" : "") + (b.mimetype ? "&mimetype=" + b.mimetype : "") + (b.filename ? "&filename=" + b.filename : "") + (b.path ? "&path=" + b.path : "") + (b.container ? "&container=" + b.container : "") + (b.access ? "&access=" + b.access : "") + (b.signature ? "&signature=" + b.signature : "") + (b.policy ? "&policy=" + b.policy : "");
    };
    var l = function (a, b) {
    ...
    return {
        BASE: b,
        DIALOG_BASE: c,
        COMM: c + "/dialog/comm_iframe/",
        FP_COMM_FALLBACK: c + "/dialog/comm_hash_iframe/",
        STORE: g,
        PICK: d,
        EXPORT: e,
        constructPickUrl: h,
        constructPickFolderUrl: i,
        constructExportUrl: j,
        constructWriteUrl: l,
        constructStoreUrl: k,
        constructHostCommFallback: m
    };
});
filepicker.extend("util", function () {
    var a = this;
...
filepicker.extend("widgets", function () {
    ...
    var d = function (d) {
        ...
        e.innerHTML = d.getAttribute('data-fp-button-text') || d.getAttribute('data-fp-text') || "Pick File";
        ...
        b("maxFiles", f, "data-fp-maxFiles", d);
        ...
        b("storeContainer", f, "data-fp-store-container", d);
        ...
        if (j) a.setKey(j);
        f.folders = (d.getAttribute("data-fp-folders") || "false") == "true";
    ...
    var e = function (d) {
        ...
        i.innerHTML = d.getAttribute('data-fp-button-text') || "Pick File";
        ...
        b("storeContainer", k, "data-fp-store-container", d);
